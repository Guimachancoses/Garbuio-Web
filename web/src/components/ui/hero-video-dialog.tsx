...
export function HeroVideoDialog({
  ...
  // Detecta YouTube e converte automaticamente para embed
  const getEmbedUrl = (url: string) => {
    ...
    // Qualquer outro link passa direto (pode ser Vimeo ou embed)
    return url
  }

  const finalVideoSrc = getEmbedUrl(videoSrc)
      ...
      <AnimatePresence>
        {isVideoOpen && (
          <motion.div
            ...
            tabIndex={0}
            onKeyDown={(e) => {
              if (["Escape", "Enter", " "].includes(e.key)) setIsVideoOpen(false)
            }}
